...
Фрази на кшталт «покажи аналітику», «воронка», «трафік», «GA4», «realtime» → `get_ga4_report` (через `maybe_direct_plan`, **без** Replicate).

1. Data API (service account) → метрики  
2. matplotlib PNG: воронка / джерела / KPI-таблиця  
3. Telegram `sendMediaGroup` + caption з цифрами  
