...
`count_orders`, `list_recent_orders`, `get_order`, `find_orders` (телефон / ім’я / місто), `count_products`, `count_in_stock_products`, `get_product_stock`, `get_ga4_report`
...
## GA4 analytics (графіки, без LLM)

Фрази: «покажи аналітику», «воронка», «трафік», «GA4», «realtime».

Dashboard = **7 PNG** в одному альбомі Telegram:
1. трафік · 2. динаміка по днях · 3. залученість · 4. воронка · 5. джерела · 6. сторінки · 7. продажі

**Автозвіт:** сервіс `ga4-weekly` у `docker-compose.prod.yml` — кожен **понеділок о 10:00 Europe/Kyiv** шле той самий дашборд у Telegram (chat з Telegram settings).
Ручний запуск: `python manage.py send_ga4_weekly_report --weekly`

Env (prod `.env`, не в git):
...
Альтернатива локально: `GOOGLE_APPLICATION_CREDENTIALS=ops/ga4/service-account.json`.

Cooldownoldown: 60 с на чат. LLM метрики не вигадує.
